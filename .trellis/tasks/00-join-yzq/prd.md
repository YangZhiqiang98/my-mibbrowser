...
`yzq` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/yzq/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/yzq/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee yzq` to
...
python ./.trellis/scripts/task.py archive 00-join-yzq
```
