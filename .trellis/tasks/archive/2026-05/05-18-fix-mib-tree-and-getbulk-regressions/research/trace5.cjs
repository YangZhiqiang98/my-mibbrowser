// Trace: Simulate the CURRENT (post-Fix-1) code where counter is NOT reset.
// But on app restart, the parser instance is fresh — counter starts at 0.
// Then loadMibCache loads cached modules with their OLD ids (from previous session).
// User then does a parseDirectory which generates fresh ids starting from 0.
// → Collision.
const fs = require('fs')
...
class Parser {
  ...
  // Note: counter NOT reset in parseFiles/parseFileContents (per Fix 1)
...
// === SCENARIO A: Single fresh parser, both BASE + ADD parsed in same session ===
// (NO id collision because counter is continuous)
console.log('=== SCENARIO A: Single parser, single session (BASE + ADD all fresh) ===')
{
  const p = new Parser()
  const base = p.parseModule(fs.readFileSync(BASE, 'utf-8'), 'BASE')
  const bert = p.parseModule(fs.readFileSync(BERT, 'utf-8'), 'BERT')
  const clocke = p.parseModule(fs.readFileSync(CLOCKE, 'utf-8'), 'CLOCKE')
  const final = buildMibTree([base, bert, clocke])

  const ce = final.find(n => n.name === 'raisecomClockEObjects')
  console.log(`raisecomClockEObjects ${ce?.id}: children=${JSON.stringify(ce?.children)}`)
  for (const cid of ce?.children ?? []) {
    const c = final.find(n => n.id === cid)
    if (c) console.log(`  - ${c.id} ${c.name} oidString=${c.oidString}`)
  }
}

// === SCENARIO B: Cache loaded from PREVIOUS session (cached nodes have ids from THAT session's counter)
//                  This session: parser starts fresh at counter=0; load cached modules; then user
//                  triggers parseDirectory which generates ids starting fresh from 0 → collision. ===
console.log('\n=== SCENARIO B: Cache (BASE) + fresh ADD parse (post-Fix-1) ===')
{
  // Previous session: only BASE was parsed there (with fresh counter)
  const prevSessionParser = new Parser()
  const cachedBase = prevSessionParser.parseModule(fs.readFileSync(BASE, 'utf-8'), 'BASE')

  // Current session: fresh parser, counter starts at 0
  const curParser = new Parser()
  // loadMibCache adds cachedBase to accumulatedModules
  // User does parseDirectory(ADD) → counter starts from 0 in curParser
  const bert = curParser.parseModule(fs.readFileSync(BERT, 'utf-8'), 'BERT')
  const clocke = curParser.parseModule(fs.readFileSync(CLOCKE, 'utf-8'), 'CLOCKE')

  // accumulatedModules = [cachedBase, bert, clocke]
  console.log('Cached BASE first/some node ids:')
  for (const i of [0, 1, 16, 17, 18, 41]) console.log(`  cachedBase[${i}] = ${cachedBase.nodes[i]?.id} ${cachedBase.nodes[i]?.name}`)
  console.log('Fresh ADD problematic node ids:')
  ...
  console.log(`raisecomClockEObjects ${ce?.id}: children=${JSON.stringify(ce?.children)}`)
  for (const cid of ce?.children ?? []) {
    // simulate dedupedMap (last-wins for collisions)
    const matches = final.filter(n => n.id === cid)
    const lastWin = matches[matches.length - 1]
    if (lastWin) console.log(`  - ${lastWin.id} ${lastWin.name} (${matches.length} matches) oidString=${lastWin.oidString}`)
  }

  // RENDERER simulation
  console.log('\n--- Renderer dedupedMap.get(cid) (LAST WIN) ---')
  const dedupedMap = new Map(final.map(n => [n.id, n]))
  console.log(`dedupedMap.size = ${dedupedMap.size} (final length = ${final.length}, ${final.length - dedupedMap.size} entries lost to collision)`)

  console.log('What rendering raisecomClockEObjects children gives:')
  if (ce) {
    for (const cid of ce.children) {
      const v = dedupedMap.get(cid)
      if (v) console.log(`  ${cid} -> ${v.name} (oid=${v.oidString})`)
      else console.log(`  ${cid} -> NOT IN dedupedMap (filtered)`)
    }
  }

  // Find where the 3 problem nodes ACTUALLY render
  console.log('\nWhere do the 3 nodes show up in dedupedMap?')
  for (const target of ['raisecomClockEDeviceMaster', 'raisecomClockESSMEnable', 'raisecomClockESrcStatusTable']) {
    const nodeInFinal = final.filter(n => n.name === target)
    for (const n of nodeInFinal) {
      const inMap = dedupedMap.get(n.id)
      const wonMap = inMap?.name === target
      console.log(`  ${target} (${n.id}): in dedupedMap as -> ${inMap?.name} (won? ${wonMap})`)
    }
  }

  // Check roots in renderer
  ...
  console.log(`\nRenderer roots count: ${roots.length}`)
  for (const r of roots) console.log(`  ROOT: ${r.id} ${r.name} parentId=${r.parentId} oidString=${r.oidString}`)
}
