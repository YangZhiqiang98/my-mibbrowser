// Final test: render full tree, check duplicate nodes
const fs = require('fs')

// reuse prev logic
function stripImportsSection(c) { return c.replace(/IMPORTS\s*[\s\S]*?;/gi, '') }
class Parser { constructor() { this.counter = 0 } parseModule(content, fileName) { const m = content.match(/^(\S+)\s+DEFINITIONS\s*::=\s*BEGIN/i); let moduleName = m ? m[1] : null; if (!moduleName) { const im = content.match(/(\S+)\s+MODULE-IDENTITY/i); moduleName = im ? im[1] : fileName } const mod = { name: moduleName, nodes: [] }; const stripped = stripImportsSection(content); let r; for (const [kind, regex] of [['OBJECT-TYPE', /(\S+)\s+OBJECT-TYPE\s*([\s\S]*?)(?::=\s*\{([^}]+)\})/g], ['OBJECT-IDENTITY', /(\S+)\s+(?:OBJECT-IDENTITY|MODULE-IDENTITY)\s*([\s\S]*?)(?::=\s*\{([^}]+)\})/g], ['NOTIFICATION-TYPE', /(\S+)\s+NOTIFICATION-TYPE\s*([\s\S]*?)(?::=\s*\{([^}]+)\})/g]]) { while ((r = regex.exec(stripped)) !== null) { mod.nodes.push({ id: `node-${++this.counter}`, name: r[1], kind, oidDef: r[3].trim(), parentId: null, children: [], oid: [], oidString: '' }) } } const oid = /(\S+)\s+OBJECT\s+IDENTIFIER\s*::=\s*\{([^}]+)\}/g; while ((r = oid.exec(stripped)) !== null) { mod.nodes.push({ id: `node-${++this.counter}`, name: r[1], kind: 'OBJECT IDENTIFIER', oidDef: r[2].trim(), parentId: null, children: [], oid: [], oidString: '' }) } return mod } }
...
for (const f of ['D:/learn/mib-cache/mib-cache-09_ros6_x_6e32ce08.json', 'D:/learn/mib-cache/mib-cache-ADD_7977868a.json']) {
  const c = JSON.parse(fs.readFileSync(f, 'utf-8'))
  const ex = new Set(accumulatedModules.map(m => m.name))
  for (const m of c.modules) if (!ex.has(m.name)) accumulatedModules.push(m)
}
const parser = new Parser()
const bert = parser.parseModule(fs.readFileSync('E:/RC/MIB/SLT8400/ADD/RAISECOM-OPT-BERT-MIB.my', 'utf-8'), 'BERT')
const clocke = parser.parseModule(fs.readFileSync('E:/RC/MIB/SLT8400/ADD/RAISECOM-OTP-CLOCKE-MIB.my', 'utf-8'), 'CLOCKE')
...
// Full renderer simulation
const dedupedNodes = final.map(n => ({ ...n, children: [...new Set(n.children)] }))
...
console.log(`=== ROOTS (renderer) ===`)
for (const r of roots) console.log(`  ${r.id} ${r.name} parentId=${r.parentId} oid=${r.oidString}`)

// Now render. Walk and find where raisecomClockE* appears.
function buildTree(node, visited) {
  const key = node.id + ':' + node.name
  if (visited.has(key)) return { ...node, children: [], cycle: true }
  const seen = new Set(visited); seen.add(key)
  return {
    id: node.id, name: node.name, parentId: node.parentId, oid: node.oidString,
    children: (node.children || []).map(c => dedupedMap.get(c)).filter(n => !!n).map(c => buildTree(c, seen))
  }
}

const tree = roots.map(r => buildTree(r, new Set()))

// Find all places where raisecomClockEDeviceMaster appears in rendered tree
function findAll(tree, predicate, path = []) {
  const results = []
  function walk(n, p) {
    if (predicate(n)) results.push({ path: [...p, n.name].join(' > '), node: n })
...
console.log(`\n=== Where do specific nodes appear in rendered tree? ===`)
for (const target of ['raisecomClockEDeviceMaster', 'raisecomClockESSMEnable', 'raisecomClockESrcStatusTable', 'raisecomClockEPllCmd', 'raisecomClockEObjects', 'raisecomClockEMib']) {
  const found = findAll(tree, n => n.name === target)
  console.log(`\n${target}: ${found.length} occurrence(s)`)
  for (const f of found) console.log(`  at: ${f.path}`)
}

// Also find any cached nodes that have raisecomClockE* children due to dedupedMap chaos
console.log(`\n=== Cached IEEE8021 nodes' rendered children (look for cross-contamination) ===`)
for (const target of ['ieee8021CfmDefaultMdTable', 'ieee8021CfmStackTable']) {
  const found = findAll(tree, n => n.name === target)
  for (const f of found) {
    console.log(`\n${target} at ${f.path}:`)
    function dump(n, d) {
      console.log(`${'  '.repeat(d)}${n.name} (${n.id})`)
