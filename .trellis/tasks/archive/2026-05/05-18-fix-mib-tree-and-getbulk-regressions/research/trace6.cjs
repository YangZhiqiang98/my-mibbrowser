// Trace: render full tree from Scenario B (cache + ADD)
const fs = require('fs')
...
// FULL RENDER TREE FROM SCENARIO B
const prev = new Parser()
const cachedBase = prev.parseModule(fs.readFileSync(BASE, 'utf-8'), 'BASE')
const cur = new Parser()
const bert = cur.parseModule(fs.readFileSync(BERT, 'utf-8'), 'BERT')
const clocke = cur.parseModule(fs.readFileSync(CLOCKE, 'utf-8'), 'CLOCKE')
const final = buildMibTree([cachedBase, bert, clocke])

// Render the FULL tree (renderer logic)
function renderTreeFromNodes(nodes) {
  const dedupedNodes = nodes.map(n => ({ ...n, children: [...new Set(n.children)] }))
  ...
  function build(node) {
    return { id: node.id, name: node.name, oid: node.oidString, parentId: node.parentId,
      children: node.children.map(c => dedupedMap.get(c)).filter(n => !!n).map(build) }
  }
  return roots.map(build)
}

const tree = renderTreeFromNodes(final)
function render(node, depth = 0) {
  console.log(`${'  '.repeat(depth)}${node.name} (${node.id}, oid=${node.oid})`)
  for (const c of node.children) render(c, depth + 1)
}
console.log('=== FULL RENDERED TREE ===')
for (const r of tree) render(r)
